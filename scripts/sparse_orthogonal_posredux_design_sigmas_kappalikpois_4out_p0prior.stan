...
//
// Also, hierarchical model such that some condition groups have non-centered gene covariates (z matrix).
data {
  ...
  real<lower=0> beta_v;
  // Hierarchical
  real<lower=0> rho_std;
  // Design matrices
  array[N_run-(N_run_2+N_run_3+N_run_4)] int<lower=1,upper=N_cond> exp_design;  // run (index) -> condition
  array[N_run_2] int<lower=1,upper=N_cond> exp_design_2;  // run (index) -> condition
  array[N_run_3] int<lower=1,upper=N_cond> exp_design_3;  // run (index) -> condition
  array[N_run_4] int<lower=1,upper=N_cond> exp_design_4;  // run (index) -> condition
  array[N_gene] int<lower=1,upper=N_ope> ope_design;  // gene (index) -> operon
  // group condition indices
  array[N_run-(N_run_2+N_run_3+N_run_4)] int<lower=1,upper=N_run> y1_index;  // gene (index) -> operon
  array[N_run_2] int<lower=1,upper=N_run> y2_index;  // gene (index) -> operon
  array[N_run_3] int<lower=1,upper=N_run> y3_index;  // gene (index) -> operon
  array[N_run_4] int<lower=1,upper=N_run> y4_index;  // gene (index) -> operon
...
parameters {
  ...
  matrix[N_gene, N_mode] z;
  matrix[N_gene, N_mode] z2;
  matrix[N_gene, N_mode] z3;
  matrix[N_gene, N_mode] z4;
...
model { 
  ...
  to_vector(z3) ~ std_normal();
  to_vector(z4) ~ std_normal();
  rho ~ cauchy(0, rho_std);
  // Likelihood
  if (likelihood == 1) {
    // beta <- U_m[O->G] * z <- Modulon matrix (modulon, gene)
    // V <- Activity matrix (modulon, condition)
    to_vector(Y[, y1_index]) ~ normal(to_vector((U_m[ope_design,] .* z * V_t)[, exp_design]), sigma);
    to_vector(Y[, y2_index]) ~ normal(to_vector((U_m[ope_design,] .* (z + z2*rho) * V_t)[, exp_design_2]), sigma);
    to_vector(Y[, y3_index]) ~ normal(to_vector((U_m[ope_design,] .* (z + z3*rho) * V_t)[, exp_design_3]), sigma);
    to_vector(Y[, y4_index]) ~ normal(to_vector((U_m[ope_design,] .* (z + z4*rho) * V_t)[, exp_design_4]), sigma);
...
generated quantities {
  ...
  matrix[N_gene, N_mode] beta = U_m[ope_design] .* z;
}
