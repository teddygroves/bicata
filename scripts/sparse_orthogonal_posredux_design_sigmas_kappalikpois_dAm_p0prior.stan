...
parameters {
  ...
  vector<lower=0>[N_cond] sigma_V;          // Standard deviation of each activity
  // sparse parameters
...
model { 
  // activity matrix
  sigma_V ~ lognormal(-0.7136, 0.1034);  // centered on 0.5 in 95 % of 0.4, 0.6
  for (cond in 1:N_cond) {
     X_V[:, cond] ~ normal(0, sigma_V[cond]);
  }
