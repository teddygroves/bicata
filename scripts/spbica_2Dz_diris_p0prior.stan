...
data {
  ...
  array[N_run_2] int<lower=1,upper=N_cond_2> exp_design_2;  // run (index) -> condition
  array[N_gene_total] int<lower=1,upper=N_ope> ope_design;  // gene (index) -> operon
  array[N_gene] int<lower=1,upper=N_gene_total> gxg_1;  // gene (index) -> operon
  array[N_gene_2] int<lower=1,upper=N_gene_total> gxg_2;  // gene (index) -> operon
...
transformed data {
  ...
  // new one (right?)
  vector[N_mode] scale_global = p0 ./ ((N_ope - p0) .* sqrt(N_cond));
  array[N_mode] int p0_int = to_int(to_array_1d(p0));
  // close compositions to 1 (they approx but not quite sum to 1) and pseudocounts
  matrix[N_gene_2, N_run_2] Y2_closed;
  for (run in 1:N_run_2) {
    Y2_closed[:,run] = (Y2[:,run]) / sum(Y2[:,run]);
  }
}
...
parameters {
  ...
  matrix<lower=0>[N_mode, N_cond_2] X_V_2;    // latent activity matrix (dataset 2)
  // sparse parameters
  vector<lower=0>[N_mode] caux;
  vector<lower=0>[N_mode] tau_sp;
  matrix[N_gene_total, N_mode] z;
  real<lower=0> alpha;
}

transformed parameters {
  // REPARAM: U ~ t-student ( nu_local, 0, 1 ), saved to compute kappa
  matrix[N_gene_total, N_mode] beta;      // sparse modulon matrix
  matrix[N_mode, N_cond] V_t;       // orthogonal activity matrix
  matrix[N_mode, N_cond_2] V_t_2;       // orthogonal activity matrix
  ...
  {   
    ...
    // Constructing V as orthogonal component of the polar decomposition of X_V
    vector[N_cond_2] eval_trans_V_2; // Transformation of eigenvalues for polar decomp.
    tuple(matrix[N_cond_2, N_cond_2], vector[N_cond_2]) evec_eval_2 = eigendecompose_sym(crossprod(X_V_2)); // eigendecomposition of z_V'*z_V (stable & eff)
    for (i in 1:N_cond_2) {
      // add an epsilon to remove numerical errors
      eval_trans_V_2[i] = 1.0 / sqrt(evec_eval_2.2[i] + 1e-10);
    }
    V_t_2 = (X_V_2 * evec_eval_2.1 * diag_matrix(eval_trans_V_2) * evec_eval_2.1');
...
model { 
  ...
  // activity matrix
  to_vector(X_V_2) ~ normal(0, beta_v);
  ...
  alpha ~ inv_gamma(0.5, 1);
  // Likelihood
  if (likelihood == 1) {
    // beta <- Modulon (modulon, gene)
    // V <- Activity matrix (modulon, condition)
    to_vector(Y) ~ normal(to_vector(beta[gxg_1] * V_t[, exp_design]), sigma);
    // second dataset
    matrix[N_gene_2, N_cond_2] y_hat_2 = (beta[gxg_2] * V_t_2);
    for (cond in 1:N_cond_2) {
      y_hat_2[:, cond] = softmax(y_hat_2[:, cond]);
    }
    for (run in 1:N_run_2) {
      Y2_closed[:, run] ~ dirichlet(y_hat_2[, exp_design_2[run]] * alpha);   
    }
    // favor a particular number of operons per modulon
    p0_int ~ poisson(meff);
  }
}
